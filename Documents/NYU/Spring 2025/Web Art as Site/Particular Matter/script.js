...
// Check if we're returning from whiteweb.html
const isReturning = sessionStorage.getItem('returning') === 'true';
...
// Find nearest web intersection
function findNearestIntersection(x, y) {
    let minDist = Infinity;
    let nearest = { ring: 0, spoke: 0 };

    for (let r = 0; r < config.rings; r++) {
        const radius = config.maxRadius * Math.pow(r / config.rings, 0.8);
        for (let s = 0; s < config.spokes; s++) {
            const angle = (Math.PI * 2 * s) / config.spokes;
            const px = config.center.x + Math.cos(angle) * radius;
            const py = config.center.y + Math.sin(angle) * radius;
            
            const dist = Math.sqrt(Math.pow(x - px, 2) + Math.pow(y - py, 2));
            if (dist < minDist) {
                minDist = dist;
                nearest = { ring: r, spoke: s };
            }
        }
    }
    return nearest;
...
function drawTransition() {
    if (config.transition.active) {
        ctx.fillStyle = `rgba(255, 255, 255, ${config.transition.progress})`;
        ctx.beginPath();
